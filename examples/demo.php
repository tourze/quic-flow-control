...
// 3. 模拟数据发送
echo "3. 模拟数据发送\n";
$sendData = function ($streamId, $bytes) use ($manager): void {
...
foreach ($streams as $streamId => $stream) {
    ...
    if ($stream->isSendBlocked()) {
        ...
        echo '     阻塞偏移量: ' . (null !== $blockedOffset ? number_format($blockedOffset) : '无') . "\n";
    }
}
...
// 5. 模拟数据接收
echo "5. 模拟数据接收\n";
$receiveData = function ($streamId, $bytes) use ($manager, $streams): void {
